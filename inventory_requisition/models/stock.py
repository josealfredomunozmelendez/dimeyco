...
class StockInventoryMaterialRequisition(models.Model):
    ...
    partner_id = fields.Many2one(
        'res.partner', string="Proveedor", default=1)
    ...
    requisition_line_ids = fields.One2many(
        'stock_inventory.material_requisition.items',
        'requisition_id', string='Materiales')
    dispatch_line_ids = fields.One2many(
        'stock_inventory.material_requisition.items',
        'requisition_id', string='Salida')
    return_line_ids = fields.One2many(
        'stock_inventory.material_requisition.items',
        'requisition_id', string='Entrada')
    ...
    def dispatch_items_button_action(self):
        ...
        for line_id in self.requisition_line_ids:
            ...
            # if sum of already disptached and dispatched is equals to line_id
            # quantity proceed to process the line
            if (
                (line_id.quantity_dispatched + line_id.quantity_dispatch) ==
                    line_id.quantity) and (line_id.status != 'stocked'):
                line_id.quantity_dispatched = (
                    line_id.quantity_dispatched + line_id.quantity_dispatch)
                line_id.status = 'stocked'
                status = 1
            elif (
                line_id.quantity < (line_id.quantity_dispatched +
                                    line_id.quantity_dispatch)):
                raise ValidationError(
                    _('No podemos entregar más de la cantidad solicitada.'))
            elif ((line_id.quantity_dispatched +
                   line_id.quantity_dispatch) > 0 and
                  (line_id.status != 'stocked')):
                line_id.quantity_dispatched = (line_id.quantity_dispatched +
                                               line_id.quantity_dispatch)
                status = 2
...
class StockInventoryMaterialItems(models.Model):
    ...
    analytic_account = fields.Many2one(
        ...
        ondelete='restrict', index=True,
        help="Los registros de este documento no afectan en tu "
        "centro de costos")
    ...
    total_weight = fields.Float(
        digits=(6, 2), string="Total Kg",
        compute='_compute_total_weight', store=True)

    def return_item(self):
        if (self.quantity_returned +
                self.quantity_return > self.quantity_dispatched):
            raise ValidationError(
                _('No se puede recibir mayor cantidad '
                  'que la que se ha prestado.'))
